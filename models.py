...
class EconomicIndicatorPredictorTwostep(nn.Module):
    def __init__(self, 
                 merge_input_dim,
                 article_embedding_dim,
                 macro_dim,
    ...
    def forward(self, financial_seq, article_seq, macro_seq):
        ...
        # Step 5: output
        output = self.mlp_head(monthly_hidden_last)
        output = self.mlp_head(monthly_hidden_last)  # (B, 2*output_dim)
        output = output.view(batch_size, 2, self.output_dim)      # (B, 2, output_dim)
        return output